...
void render() {
 ...
 gluLookAt(cx, 4.0,cy,ccx, 0.5, ccy, 0., 1., 0.);
      ...
      if (e.type == Expose){
          ...
          if (e.type == KeyPress){
			...
			if(keysym == XK_Left || keysym == XK_Right || keysym == XK_Up || keysym == XK_Down){
				key=0;
				if (keysym == XK_Left) xx=x0-1.0f;
				if (keysym == XK_Left) key=1;
		
				if (keysym == XK_Right) xx=x0+1.0f;
				if (keysym == XK_Right) key=2;
				
				if (keysym == XK_Up) yy=y0-1.0f;
				if (keysym == XK_Up) key=3;
				
				if (keysym == XK_Down) yy=y0+1.0f;
				if (keysym == XK_Down) key=4;
